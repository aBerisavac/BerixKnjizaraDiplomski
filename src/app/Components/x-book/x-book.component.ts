...
export class XBookComponent implements OnInit {
  ...
  public setListItemLogic(listHolder: any){
    let list = listHolder.nativeElement.querySelector(".list") as HTMLDivElement;
    let infoHolder = listHolder.nativeElement.querySelector(".info-holder") as HTMLDivElement;
      infoHolder.addEventListener("click", (e)=>{
        if(parseInt(list.style.top)<0){
          list.style.top =infoHolder.clientHeight+5+"px";
          (listHolder.nativeElement as HTMLDivElement).style.paddingBottom = list.clientHeight+"px"
        } else{
          list.style.top = -list.clientHeight-5 + infoHolder.clientHeight+"px";
          (listHolder.nativeElement as HTMLDivElement).style.paddingBottom = 0+"px"
        }
      })
